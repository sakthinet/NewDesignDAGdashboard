...
def generate_sip_dag(
    ...
    def get_attachment_paths_from_csv(data: dict):
        try:
            ...
            print(f"❌ Error in get_attachment_paths_from_csv: {e}")
            raise
    ...
    raw_pdi_xml = build_pdi_xml(processed, holding_name, root_element, child_element)
    pretty_pdi_xml = prettify_xml(raw_pdi_xml)
    pdi_xml_path = os.path.join(OUTPUT_FOLDER, "eas_pdi.xml")
    pdi_written = write_to_file(pretty_pdi_xml, pdi_xml_path)

    # Ensure AIU count uses the actual written file path and waits for write_to_file
    ...
    attachments = get_attachment_paths_from_csv(csv_data)
    ...
    sip_xml_path = os.path.join(OUTPUT_FOLDER, "eas_sip.xml")
    sip_written = write_to_file(pretty_sip_xml, sip_xml_path)

    zip_task = create_sip_zip(
        holding_name=holding_name,
    ...
    # Set dependencies: pdi_written >> aiu_count >> pdi_valid >> attachments >> sip_written >> zip_task
    pdi_written >> aiu_count >> pdi_valid >> attachments >> sip_written >> zip_task
