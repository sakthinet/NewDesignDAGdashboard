...
def get_runtime_config():
    ...
    return {
        ...
        "record_count":     params.get("Records Count", 1000000),
...
def is_float(val):
    try:
        f = float(val)
        return "." in str(val) or not f.is_integer()
    except:
        return False
...
with DAG(
    dag_id="InfoArchiveSchemaGenerator",
    ...
    def analyze_all_csvs():
        ...
        for fn in os.listdir(CSV_FOLDER):
            ...
            df   = next(pd.read_csv(path, chunksize=10000, dtype=object))
            states = {c: {"date": False, "float": False, "string": False, "done": False}
                      for c in df.columns}

            for chunk in [df] + list(pd.read_csv(path, chunksize=10000, dtype=object)):
    ...
    def generate_metadata_xml(table_infos):
        ...
        for info in table_infos:
            ...
            ET.SubElement(tbl, "recordCount").text = str(cfg["record_count"])
